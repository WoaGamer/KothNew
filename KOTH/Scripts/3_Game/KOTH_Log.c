class KOTH_Log {
    ...
    protected static string m_RootDirectory = "$profile:KOTH";
    ...
    protected static string GetDate() {
        ...
        if (m_LastDay != m_Day) m_Date = string.Format("%1-%2-%3", m_Year.ToStringLen(2), m_Month.ToStringLen(2), m_Day.ToStringLen(2));
    ...
    protected static void WriteLine(string input) {
        if (!FileExist(m_RootDirectory)) MakeDirectory(m_RootDirectory);

        if (!FileExist(m_LogDirectory)) MakeDirectory(m_LogDirectory);
    ...
    static void LogBasic(string message) {
        if (KOTH_Settings.GetLoggingLevel() >= eKOTH.LOG_BASIC) WriteLine(string.Format("%1", message));
    }

    static void LogCritical(string message) {
        if (KOTH_Settings.GetLoggingLevel() >= eKOTH.LOG_CRITICAL) WriteLine(string.Format("[CRITICAL] %1", message));
    }
    ...
    static void LogVerbose(string message) {
        if (KOTH_Settings.GetLoggingLevel() >= eKOTH.LOG_VERBOSE) WriteLine(string.Format("%1", message));
    }
}
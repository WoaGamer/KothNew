class KOTH_ContaminatedArea extends ContaminatedArea_Dynamic {
    ...
    void OnCreate() {
        if (GetGame().IsServer()) m_PPERequesterIdx = GetRequesterIndex(m_PPERequesterType);
    ...
    void OnUpdate() {
        if (GetRemainingTime() < GetFinishDecayLifetime()) SetDecayState(eAreaDecayStage.DECAY_END);
        else if (GetRemainingTime() < GetStartDecayLifetime()) SetDecayState(eAreaDecayStage.DECAY_START);
    ...
    override void EEInit() {
        if (!m_ToxicClouds) m_ToxicClouds = new array < Particle > ;
        ...
        if (m_DecayState == eAreaDecayStage.LIVE) InitZone();
        else if (GetGame().IsClient() && m_DecayState > eAreaDecayStage.LIVE) InitZoneClient();
